...
import '../http/byte_stream.dart';
import '../http/client_exception.dart';
import '../http/multipart_file.dart';
import '../http/multipart_request.dart';
import '../utils/types.dart';


Future<T> handlePlatformRequest<T>(Method method, String path,
  ...
  var ioRequest =
  await client.openUrl(method.name, Uri.parse(path));
  ...
  if (_responseIsOk(response.statusCode)) {
    switch (resolveType) {
      case ResolveType.json:
        if (responseBody.isEmpty) return null as T;
        return json.decode(utf8.decode(responseBody)) as T;
      case ResolveType.text:
        return utf8.decode(responseBody) as T;
      case ResolveType.bytes:
...
Future<T> handlePlatformUpload<T>(
    String path, Object body, String fileName, String contentType,
    {required Map<String, dynamic> query,
      required Map<String, dynamic> headers,
      void Function(int loaded, int total, double percent)? onProgress}) async {
